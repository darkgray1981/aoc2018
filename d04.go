...
func main() {
	...
	for _, s := range arr {
		...
		if strings.Contains(s, "Guard") {
			fmt.Sscanf(s, "%s %s %s #%d", &dummy, &dummy, &dummy, &guard)
			continue
		}

		minute := -1

		fmt.Sscanf(s, "%s %2s:%d", &dummy, &dummy, &minute)
	...
	for id, schedule := range sleeping {
		min := 0
		total := 0
	...
	fmt.Println("Done:", mostId*mostMinute, time.Since(t))
	...
	for id, schedule := range sleeping {
		...
		if schedule[min] > mostTotal {
			mostId = id
			mostTotal = schedule[min]
			mostMinute = min
		}
	}

	fmt.Println("Done 2:", mostId*mostMinute, time.Since(t))
}

const INPUT1 = `[1518-11-01 00:00] Guard #10 begins shift
...
const INPUT = `[1518-05-12 00:46] wakes up
